...
async function main() {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ action: 'create', email: EMAIL, password: PASSWORD, full_name: NAME })
  })
  
  const data = await res.json()
  console.log('Lovable resposta:', JSON.stringify(data))
  ...
  await supabase.from('email_logs').insert({
    ...
    metadata: { user_email: EMAIL, order_value: 36, payment_method: 'pix' }
  })
  
  console.log('\n========================================')
  console.log('CREDENCIAIS DO WEVERTON:')
  console.log('Email:', EMAIL)
